...
export default function DashboardPage() {
  ...
  async function handleSubmitArticle(e: React.FormEvent) {
    ...
    if (!formTitle.trim() || !formContent.trim()) {
      toast.warning("Form Tidak Lengkap", {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(editingId ? "Artikel diperbarui!" : "Artikel diterbitkan!", {
          description: editingId ? "Perubahan berhasil disimpan." : "Artikel baru berhasil diterbitkan.",
        });
        resetForm();
        await fetchArticles();
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Berita dihapus!", {
          description: "Konten berhasil dihapus dari database.",
        });
        await fetchArticles();
      } else {
        const data = await res.json();
